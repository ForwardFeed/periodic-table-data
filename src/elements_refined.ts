...
import { Block, Color, CrystalStructure, DecayMode, ElectricalType, GasAtomicMultiplicities, MagneticType, Phase, Series, UNITS, type IsotopicAbundance, type MaybeNumber, type MeasurementUnits } from "./units";
...
function to_n_mu(str: string, units: MeasurementUnits | MeasurementUnits[] | readonly MeasurementUnits[]): MaybeNumber{
    if (typeof units === "string"){
        units = [units]
    }
    const no_unit_str = units.reduce((cumu, curr)=>{
        return cumu.replace(curr, '') as MeasurementUnits
    }, str as MeasurementUnits)
